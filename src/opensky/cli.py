...
from opensky.config import VALID_CABINS, VALID_STOPS
from opensky.search import ProviderFailure

app = typer.Typer(
    name="opensky",
    help="Flight search with itinerary and overflight conflict-zone flagging.",
...
def _validate_provider(provider: str | None) -> None:
    if provider is not None and provider not in VALID_PROVIDERS:
        ...
        stderr_console.print(
            "[dim]Env vars: OPENSKY_DUFFEL_TOKEN, OPENSKY_AMADEUS_KEY + OPENSKY_AMADEUS_SECRET "
            "(fallback: SKYROUTE_DUFFEL_TOKEN, SKYROUTE_AMADEUS_KEY + SKYROUTE_AMADEUS_SECRET)[/dim]"
        )
        raise typer.Exit(1)
...
def zones(
    ...
    if update:
        ...
        url = "https://raw.githubusercontent.com/buildingopen/opensky/master/src/opensky/data/conflict_zones.json"
