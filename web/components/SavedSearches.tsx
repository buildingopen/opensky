  ...
  return (
    <div className="mt-4">
      <div className="flex items-center justify-between mb-2">
        ...
        <button onClick={onClear} className="text-[11px] text-[var(--color-text-muted)]/50 hover:text-[var(--color-text-muted)] transition-colors" aria-label="Clear search history">
          Clear
        </button>
      </div>
      <div className="flex flex-wrap gap-2">
        {searches.map((s) => (
          <button
            ...
            className="text-[13px] px-3.5 py-1.5 rounded-full bg-[var(--color-surface-2)] text-[var(--color-text-muted)] hover:bg-[var(--color-border)] hover:text-[var(--color-text)] transition-all duration-150 whitespace-nowrap max-w-[200px] truncate"
          >
            {s.query}
          </button>
        ))}
